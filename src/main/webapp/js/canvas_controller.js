...
function render_grid() {
    ...
    if (canvas.getContext) {
        ...
        for (let i = stepX; i < width; i += stepX) {
            ...
            let digit = Math.round(-stepCount / 2 + i / stepX)
            digit = digit === 0 ? "" : digit
        ...
        for (let i = stepY; i < height; i += stepY) {
            ...
            let digit = Math.round(stepCount / 2 - i / stepY)
            digit = digit === 0 ? "" : digit
...
function render_target(r) {
    ...
    if (canvas.getContext) {
        ...
        ctx.fillStyle = "blue"
        ctx.globalAlpha = 0.5
        ctx.fillRect(width / 2, height / 2 + stepY * (r/2), stepX * r, - stepY * r/2)
        ctx.beginPath()
        ctx.arc(width / 2, height / 2, r * stepX, Math.PI, Math.PI * 3 / 2, false)
